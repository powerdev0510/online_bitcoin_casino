...
if(isset($user) && isset($user_id) && isset($amount)){  
  ...
  if($user_id == $user_data["user_id"]){
    //Continue If Has Investment
    if($amount <= $user_data["invested"]){  
      ...
      print('{"action": true, "message": "You Divested ฿' . number_format($amount, 8, '.', '') . '!"}');
    } else {
      print('{"action": false, "message": "You Can\'t Divest More Than Your Balance of: ฿' . number_format($user_data["balance"], 8, '.', '') . '"}');
    }
  } else {
    //Not Valid
